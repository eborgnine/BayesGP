...
firstMomentAllList = parallel::mcmapply(forImportanceSampling,
...
ffFixedTheta <- TMB::MakeADFun(
      data = c(theDens$env$data, list(theta = res$mod$modesandhessians$theta[Dconfig])),
...
```{r allSamplesPlot, eval=TRUE, fig.cap = 'marginals v joint', fig.subcap = c('joint','m1','m2')}

lims1 = quantile(allSamples[,51], c(0.05, 0.95))
lims2 = quantile(allSamples[,52], c(0.05, 0.95))

theDensK = MASS::kde2d(allSamples[,51], allSamples[, 52], h=0.3, n=300,
  lims = c(lims1, lims2))
...
```{r allHIst, fig.height=4, fig.width=4, fig.cap='mcmc (hist), tmb (red), recalculated (blue)', fig.subcap = 1:ncol(allSamples)}
