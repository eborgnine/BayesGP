...
fit_result <- model_fit(
  ...
  + score + f(
      smoothing_var = year,
      model = "IWP",
      order = polyOrder2, k = 1, # should add a checker for k >= 3
...
global_samp_indexes <- fit_result$global_samp_indexes
coef_samp_indexes <- fit_result$coef_samp_indexes
fixed_samp_indexes <- fit_result$fixed_samp_index

## Ideally, we could name each of the indices vector:
## i.e. coefsamps1 <- samps$samps[coef_samp_indexes[fit_result$coef_samp_indexes$floor.size, ]
## i.e. global_samps1 <- samps$samps[coef_samp_indexes[fit_result$global_samp_indexes$floor.size, ]
## i.e. coefsamps2 <- samps$samps[coef_samp_indexes[fit_result$coef_samp_indexes$year, ]
## i.e. fixed_samps <- samps$samps[coef_samp_indexes[fit_result$fixed_samp_indexes$location, ]


global_samps1 <- samps$samps[global_samp_indexes[[1]], , drop = F]
coefsamps1 <- samps$samps[coef_samp_indexes[[1]], ]

global_samps2 <- samps$samps[global_samp_indexes[[2]], , drop = F]
coefsamps2 <- samps$samps[coef_samp_indexes[[2]], ]

fixed_samps <- samps$samps[fixed_samp_indexes, , drop = F]
