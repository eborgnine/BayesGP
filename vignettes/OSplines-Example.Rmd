---
title: "OSplines: Basic Example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{OSplines: Basic Example}
...
```{r}
X <- global_poly(x = data$t, p = polyOrder)
dim(X)
```
...
Some examples of the TMB templates written in cpp file format can be found and compiled at:

```{r}
### Template for Gaussian response:
system.file("extsrc/OSplines.cpp", package="OSplines")
```

```{r eval=FALSE}
### compiling and loading the template:
### To run while in the directory with the file "OSplines.cpp":
compile("OSplines.cpp")
dyn.load(dynlib("OSplines"))
```

For the inference, let's use the pre-compiled template of TMB/AGHQ:
...
f1derivpos %>% ggplot(aes(x = x)) + geom_line(aes(y = mean), color = "blue") + 
  ...
  theme_classic() + xlab("year since 1981") + ylab("rent: second derivative") +
